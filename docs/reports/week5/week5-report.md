...
| **4. Web UI Dashboard 4 Tabs** | `agent/ui.py` | • Tab 1: AI Agent (HITL Card & 4 Presets).<br>• Tab 2: Manual Tester (Đánh giá rủi ro trước gửi).<br>• Tab 3: Audit Logs (.jsonl).<br>• Tab 4: Guardrails & Live Injection/PII Probe. | Giao diện DevSecOps trực quan, realtime, quản trị an toàn chuyên sâu. |
