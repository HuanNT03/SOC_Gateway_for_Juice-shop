...
File: tools/test_redactor.py
Description: Kiểm thử độc lập bộ lọc mask_sensitive_data() với các dữ liệu mẫu:
             - Key nhạy cảm trong Header & JSON (x-api-key, apikey, authorization, set-cookie, v.v.)
             - Quét đệ quy trong JSON lồng nhau (nested dicts & lists)
             - Biểu thức chính quy (Regex) làm sạch Email & JWT Token thuần
"""
...
# Nạp module redactor từ thư mục tools
